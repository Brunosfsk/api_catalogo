...
export class OrdersController {
  async getOrders(req, res) {
    const { name_bussiness } = req.query

    const q = `
      SELECT orders_business.id_business, orders_business.name, orders_business.description, orders_business.price, orders_business.photo_thumb 
    ...
    `
    if (!name_bussiness) return res.json({ error: 'informação base não foi enviada' })

    db.query(q, [name_bussiness], (err, data) => {
